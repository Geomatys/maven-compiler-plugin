...
public class CompilerMojo extends AbstractCompilerMojo {
    ...
    protected void addImplicitDependencies(Map<PathType, List<Path>> addTo, boolean hasModuleDeclaration)
        ...
        if (multiReleaseOutput) {
            ...
            /*
             * Find the module name. It many module-info classes are found,
            ...
            if (moduleName == null) {
                for (Path path : getCompileSourceRoots()) {
                    path = path.resolve(MODULE_INFO + JAVA_FILE_SUFFIX);
                    if (Files.exists(path)) {
                        Charset charset = charset();
                        try (BufferedReader in = (charset != null)
