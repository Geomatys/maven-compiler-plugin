...
class ForkedTool implements Tool, OptionChecker {
    /**
     * The directory to run the compiler from.
     */
    ...
    ForkedTool(final AbstractCompilerMojo mojo) {
        ...
        executable = mojo.executable;
        debugFileName = mojo.getDebugFileName();
    }
    ...
    final boolean run(
        ...
        for (JavaFileObject source : compilationUnits) {
            command.add(fileManager.asPath(source).toString());
        }
    ...
    private int start(ProcessBuilder builder, Appendable out) throws IOException {
        if (debugFileName != null && !debugFileName.isEmpty()) {
            Path p = Path.of(debugFileName);
            if (basedir != null) {
                p = basedir.resolve(p);
            }
            Files.write(p, builder.command());
        }
