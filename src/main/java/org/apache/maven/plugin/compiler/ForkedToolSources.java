...
final class ForkedToolSources implements StandardJavaFileManager {
     ...
     */
    private final Map<JavaPathType, Collection<? extends Path>> locations;
    ...
    public void setLocationFromPaths(Location location, Collection<? extends Path> paths) {
        Optional<JavaPathType> type = JavaPathType.valueOf(location);
        if (type.isPresent()) {
            if (paths != null) {
                locations.put(type.get(), paths);
            } else {
                locations.remove(type.get());
            }
        } else if (paths != null) {
    ...
    void addAllLocations(List<String> command) {
        for (Map.Entry<JavaPathType, Collection<? extends Path>> entry : locations.entrySet()) {
