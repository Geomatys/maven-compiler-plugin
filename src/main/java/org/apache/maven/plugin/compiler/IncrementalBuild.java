...
final class IncrementalBuild {
        ...
        static EnumSet<Aspect> parse(final String values) {
            ...
            for (String value : values.split(",")) {
                ...
                try {
                    ...
                    throw new MojoException(sb.append('.').toString(), e);
                }
            }
            for (Aspect aspect : aspects) {
                for (Aspect exclude : aspect.excludes) {
                    if (aspects.contains(exclude)) {
                        throw new MojoException("Illegal incremental build setting: \"" + aspect + "\" and \"" + exclude
                                + "\" are mutually exclusive.");
                    }
                }
            }
            if (aspects.isEmpty()) {
                throw new MojoException("Illegal incremental build setting cannot be empty.");
            }
            return aspects;
        }
    }
     ...
     *
     * @see AbstractCompilerMojo#cacheFile
     */
     ...
     */
    IncrementalBuild(AbstractCompilerMojo mojo, List<SourceFile> sourceFiles) {
        this.sourceFiles = sourceFiles;
        cacheFile = mojo.cacheFile;
    ...
    String inputFileTreeChanges(final long staleMillis, final boolean rebuildOnAdd) throws IOException {
        ...
        for (SourceFile source : sourceFiles) {
            ...
            if (previous != null) {
                if (source.lastModified - previous.lastModified <= staleMillis) {
                    allChanged = false;
                    continue; // Source file has not been modified.
                }
         ...
         */
        StringBuilder causeOfRebuild = causeOfRebuild("added or removed source files", showCompilationChanges);
        if (showCompilationChanges) {
     ...
     * @see Aspect#OPTIONS
     */
    String optionChanges(int optionsHash) {
        if (optionsHash == previousOptionsHash) {
            return null;
        }
        return causeOfRebuild("changes in compiler options", false).toString();
    }
    ...
    private static StringBuilder causeOfRebuild(String cause, boolean colon) {
        return new StringBuilder(128)
                .append("Recompiling the project because of ")
                .append(cause)
                .append(colon ? ':' : '.');
    }
    ...
    String markNewOrModifiedSources(long staleMillis, boolean rebuildOnAdd) throws IOException {
        ...
        for (SourceFile source : sourceFiles) {
            if (!source.isNewOrModified) {
                ...
                if (Files.exists(output, options)) {
                    FileTime t = Files.getLastModifiedTime(output, options);
                    if (source.lastModified - t.toMillis() <= staleMillis) {
                        continue;
                    }
                } else if (rebuildOnAdd) {
                    StringBuilder causeOfRebuild = causeOfRebuild("added source file", showCompilationChanges);
                    if (showCompilationChanges) {
                        causeOfRebuild
                                .append(System.lineSeparator())
                                .append("  + ")
